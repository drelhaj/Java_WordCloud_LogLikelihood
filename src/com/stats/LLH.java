...
public int getCorpusSizse(){
	...
	public static void main(String[] args) throws IOException{
		...
		String ref = "E:/workspace/Loglikelihood/test/ref.txt";
		String src = "E:/workspace/Loglikelihood/test/src.txt";
		String word = "finance";
		LLH llhRef = countWordFreq(word, Paths.get(ref));
		LLH llhSrc = countWordFreq(word, Paths.get(src));		        

        
		double logLL = logLikeliHood(llhSrc.wordCount, llhRef.wordCount, llhSrc.corpusSize,  llhRef.corpusSize);
		System.out.println("LLH of: " + word + " = " + logLL);
	}
		...
		//create a file channel and buffer. Here I use Non-blocking I/O libraries needed when dealing with intensive input/output.
				...
				while (noOfBytesRead != -1) {
		           ...
		           wordCount += getCorpusSize(text);
					...
					//iterates over text to extract words through detecting boundaries in text to get corpus size in words.
...
/**
 * 	
 * @param word1Freq frequency of the word in your corpus
 * @param word2Freq frequency of the word in the reference corpus
 * @param corpus1Size size of your corpus in words
 * @param corpus2Size size of the reference corpus in words
 * @return
 */
	public static double logLikeliHood(int word1Freq, int word2Freq, int corpus1Size, int corpus2Size){
		...
		if(ExpectedValue1==0 || ExpectedValue2==0 || Double.isNaN(ExpectedValue1) || Double.isNaN(ExpectedValue1) ||
			Double.isNaN(Math.log(word1Freq/ExpectedValue1)) || Double.isNaN(Math.log(word2Freq/ExpectedValue2))||
			Double.isInfinite(Math.log(word1Freq/ExpectedValue1)) || Double.isInfinite(Math.log(word2Freq/ExpectedValue2))){
			llh=0.0;
		}
		else{
